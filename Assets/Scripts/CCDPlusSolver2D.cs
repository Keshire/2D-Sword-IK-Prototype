...
{
    [Solver2DMenuAttribute("Chain (CCD) Plus")]
    public class CCDPlusSolver2D : Solver2D
    {
        ...
        [SerializeField]
        [Range(kMinIterations, 50)]
        private int m_Iterations = 10;
        [SerializeField]
        [Range(kMinTolerance, 0.1f)]
        ...
        {
            Profiler.BeginSample("CCDPlusSolver2D.DoUpdateIK");
            ...
            if (CCD2D.Solve(effectorPosition, GetPlaneRootTransform().forward, iterations, tolerance, Mathf.Lerp(kMinVelocity, kMaxVelocity, m_Velocity), ref m_Positions))
            {
                for (int i = 0; i < m_Chain.transformCount - 1; ++i)
                {
                    ...
                    if (m_Chain.transforms[i].GetComponent<BoneAngleLimits>())
                    {
